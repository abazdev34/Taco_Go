...
const CashierMonitor = () => {
	...
	const renderAcceptTab = () => (
		<div className='cashier-layout'>
			...
			<section className='cashier-panel cashier-menu-panel'>
				<div className='cashier-panel-toolbar cashier-panel-toolbar--between cashier-panel-toolbar--accept'>
					...
					<div className='cashier-accept-top-actions'>
						...
						<button
							type='button'
							className='cashier-order-side-btn cashier-order-side-btn--accept'
			...
			<aside className='cashier-panel cashier-order-panel'>
				<div className='cashier-panel-heading'>
					<h3>Оформление заказа</h3>
				</div>

				<div className='cashier-order-panel__scroll'>
					<div className='cashier-order-grid-pairs'>
				...
				{!session ? (
					<div className='cashier-cash-form' style={{ marginBottom: 16 }}>
						<input
							...
							value={openAmount}
							onChange={(e) => setOpenAmount(e.currentTarget.value.replace(/\D/g, ''))}
						/>
						...
						<button
							...
							onClick={async () => {
								try {
									...
									alert(
										`Смена открыта: ${new Date(created.opened_at).toLocaleString('ru-RU')}`
									)
								} catch (e: any) {
						...
						<button
							...
							onClick={() =>
								generateCashReportPdf({
									...
									totalOrdersAmount:
										cashboxStats.cashOrdersTotal + cashboxStats.onlineOrdersTotal,
