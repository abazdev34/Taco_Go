...
function HallMonitor() {
      ...
      try {
        ...
        const prepared = (data || [])
          ...
          .filter((item: THallMenuItem) => {
            ...
            return categoryName.includes("популяр") || categoryName.includes("popular");
          })
  ...
  const readyOrders = useMemo(() => {
    return safeOrders
      .filter((order) => order.status === "ready" && getOrderPlaceValue(order) === "hall")
      .sort((a, b) => {
  ...
  const playReadySound = () => {
    try {
      ...
      oscillator.frequency.linearRampToValueAtTime(1046, audioCtx.currentTime + 0.16);
      oscillator.frequency.linearRampToValueAtTime(1318, audioCtx.currentTime + 0.32);
            ...
            <div className="hall-number-list">
              {loading ? (
                ...
                preparingOrders.map((order) => (
                  <span className="hall-number hall-number--preparing" key={order.id}>
                ...
                readyOrders.map((order) => (
                  <span
                    className={`hall-number hall-number--ready ${
                      highlightedIds.includes(order.id) ? "hall-number--active" : ""
                    }`}
                    key={order.id}
                  >
                    {getDisplayNumber(order)}
